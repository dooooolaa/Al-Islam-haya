...
const QiblaPage = () => {
  ...
  const handleDeviceOrientation = (event: DeviceOrientationEvent) => {
    ...
    if ('webkitCompassHeading' in event) {
      ...
      // Try using alpha directly first, as 360 - alpha might be incorrect on some devices
      heading = event.alpha;
    } else if (event.alpha !== null) {
      // Fallback for other devices (non-absolute or without webkitCompassHeading)
      // Again, try using alpha directly
      heading = event.alpha;
    }
    ...
    if (heading !== null) {
      ...
      if (compassRef.current) {
        // Rotate the compass background opposite to the device's heading
      ...
      if (qiblaData && qiblaArrowRef.current) {
        // Calculate the relative angle from the current heading to the Qibla direction
        // The arrow should point from the device's current orientation towards the Qibla.
        // The angle difference needed is Qibla - Current Heading.
        let relativeQiblaAngle = qiblaData.qiblaDirection - heading;

        // Normalize the relative angle
        relativeQiblaAngle = (relativeQiblaAngle + 360) % 360;
