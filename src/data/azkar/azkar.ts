export const azkarData = [
  {
    id: '1',
    text: 'سُبْحَانَ اللَّهِ وَبِحَمْدِهِ',
    count: 100,
    audio: 'https://audio.islamweb.net/audio/index.php?page=fullcontent&audioid=13756',
    category: 'أذكار الصباح',
    explanation: 'من قالها مائة مرة حُطت خطاياه وإن كانت مثل زبد البحر'
  },
  {
    id: '2',
    text: 'لا إله إلا الله وحده لا شريك له، له الملك وله الحمد وهو على كل شيء قدير',
    count: 100,
    audio: 'https://audio.islamweb.net/audio/index.php?page=fullcontent&audioid=13757',
    category: 'أذكار الصباح',
    explanation: 'كانت له عدل عشر رقاب، وكتبت له مائة حسنة، ومحيت عنه مائة سيئة، وكانت له حرزاً من الشيطان'
  },
  {
    id: '12',
    text: "أَصْبَحْنَا وَأَصْبَحَ الْمُلْكُ لِلَّهِ، وَالْحَمْدُ لِلَّهِ، لَا إِلَهَ إِلَّا اللَّهُ وَحْدَهُ لَا شَرِيكَ لَهُ، لَهُ الْمُلْكُ وَلَهُ الْحَمْدُ وَهُوَ عَلَى كُلِّ شَيْءٍ قَدِيرٌ",
    count: 1,
    category: 'أذكار الصباح',
    explanation: 'ذكر الصباح الذي كان النبي ﷺ يداوم عليه'
  },
  {
    id: '13',
    text: "اللَّهُمَّ بِكَ أَصْبَحْنَا، وَبِكَ أَمْسَيْنَا، وَبِكَ نَحْيَا، وَبِكَ نَمُوتُ، وَإِلَيْكَ النُّشُورُ",
    count: 1,
    category: 'أذكار الصباح',
  },
  {
    id: '3',
    text: 'سُبْحَانَ اللَّهِ وَبِحَمْدِهِ، سُبْحَانَ اللَّهِ الْعَظِيمِ',
    count: 100,
    audio: 'https://audio.islamweb.net/audio/index.php?page=fullcontent&audioid=13758',
    category: 'أذكار المساء',
    explanation: 'ثقيلتان في الميزان، حبيبتان إلى الرحمن'
  },
  {
    id: '4',
    text: 'أَسْتَغْفِرُ اللَّهَ وَأَتُوبُ إِلَيْهِ',
    count: 100,
    audio: 'https://audio.islamweb.net/audio/index.php?page=fullcontent&audioid=13759',
    category: 'أذكار المساء',
    explanation: 'من قالها مائة مرة في اليوم غفرت ذنوبه وإن كانت مثل زبد البحر'
  },
  {
    id: '14',
    text: "أَمْسَيْنَا وَأَمْسَى الْمُلْكُ لِلَّهِ، وَالْحَمْدُ لِلَّهِ، لَا إِلَهَ إِلَّا اللَّهُ وَحْدَهُ لَا شَرِيكَ لَهُ، لَهُ الْمُلْكُ وَلَهُ الْحَمْدُ وَهُوَ عَلَى كُلِّ شَيْءٍ قَدِيرٌ",
    count: 1,
    category: 'أذكار المساء',
  },
  {
    id: '5',
    text: 'اللَّهُمَّ أَنْتَ رَبِّي لا إِلَهَ إِلا أَنْتَ، خَلَقْتَنِي وَأَنَا عَبْدُكَ، وَأَنَا عَلَى عَهْدِكَ وَوَعْدِكَ مَا اسْتَطَعْتُ، أَعُوذُ بِكَ مِنْ شَرِّ مَا صَنَعْتُ، أَبُوءُ لَكَ بِنِعْمَتِكَ عَلَيَّ، وَأَبُوءُ لَكَ بِذَنْبِي فَاغْفِرْ لِي؛ فَإِنَّهُ لا يَغْفِرُ الذُّنُوبَ إِلا أَنْتَ',
    count: 1,
    audio: 'https://audio.islamweb.net/audio/index.php?page=fullcontent&audioid=13760',
    category: 'أذكار الصباح والمساء',
    explanation: 'سيد الاستغفار ومن قاله موقناً به حين يصبح أو يمسي فمات دخل الجنة'
  },
  {
    id: '6',
    text: 'بِسْمِ اللَّهِ الَّذِي لا يَضُرُّ مَعَ اسْمِهِ شَيْءٌ فِي الأَرْضِ وَلا فِي السَّمَاءِ وَهُوَ السَّمِيعُ الْعَلِيمُ',
    count: 3,
    audio: 'https://audio.islamweb.net/audio/index.php?page=fullcontent&audioid=13761',
    category: 'أذكار الصباح والمساء',
    explanation: 'من قالها ثلاثاً لم يضره شيء'
  },
  {
    id: '7',
    text: 'أَعُوذُ بِكَلِمَاتِ اللَّهِ التَّامَّاتِ مِنْ شَرِّ مَا خَلَقَ',
    count: 3,
    audio: 'https://audio.islamweb.net/audio/index.php?page=fullcontent&audioid=13762',
    category: 'أذكار الصباح والمساء',
    explanation: 'من قالها ثلاثاً لم يضره شيء بإذن الله'
  },
  {
    id: '8',
    text: 'اللَّهُمَّ إِنِّي أَسْأَلُكَ الْعَافِيَةَ فِي الدُّنْيَا وَالآخِرَةِ، اللَّهُمَّ إِنِّي أَسْأَلُكَ الْعَفْوَ وَالْمُعَافَاةَ فِي دِينِي وَدُنْيَايَ وَأَهْلِي وَمَالِي، اللَّهُمَّ اسْتُرْ عَوْرَاتِي، وَآمِنْ رَوْعَاتِي، اللَّهُمَّ احْفَظْنِي مِنْ بَيْنِ يَدَيَّ، وَمِنْ خَلْفِي، وَعَنْ يَمِينِي، وَعَنْ شِمَالِي، وَمِنْ فَوْقِي، وَأَعُوذُ بِعَظَمَتِكَ أَنْ أُغْتَالَ مِنْ تَحْتِي',
    count: 1,
    audio: 'https://audio.islamweb.net/audio/index.php?page=fullcontent&audioid=13763',
    category: 'أذكار الصباح والمساء',
    explanation: 'دعاء عظيم جامع لخيري الدنيا والآخرة والوقاية من الشرور'
  },
  {
    id: '15',
    text: "بِاسْمِكَ رَبِّي وَضَعْتُ جَنْبِي، وَبِكَ أَرْفَعُهُ، إِنْ أَمْسَكْتَ نَفْسِي فَارْحَمْهَا، وَإِنْ أَرْسَلْتَهَا فَاحْفَظْهَا بِمَا تَحْفَظُ بِهِ عِبَادَكَ الصَّالِحِينَ",
    count: 1,
    category: 'أذكار النوم',
  },
  {
    id: '9',
    text: 'الْحَمْدُ لِلَّهِ الَّذِي أَحْيَانَا بَعْدَ مَا أَمَاتَنَا وَإِلَيْهِ النُّشُورُ',
    count: 1,
    audio: 'https://audio.islamweb.net/audio/index.php?page=fullcontent&audioid=13764',
    category: 'أذكار الاستيقاظ',
    explanation: 'ذكر يقال عند الاستيقاظ من النوم شكراً لله على نعمة الحياة بعد الموت الأصغر (النوم)'
  },
  {
    id: '10',
    text: "اللَّهُمَّ اغْفِرْ لِي، وَارْحَمْنِي، وَعَافِنِي، وَارْزُقْنِي",
    count: 3,
    audio: "https://audio.islamweb.net/audio/index.php?page=fullcontent&audioid=13765",
    category: "أدعية متنوعة",
    explanation: 'دعاء جامع لخيري الدنيا والآخرة، يسأل فيه العبد ربه المغفرة والسعة في الدار والبركة في الرزق'
  },
  {
    id: '11',
    text: "اللَّهُمَّ اجْعَلْنَا مِنْ عَفَوْتَ وَرَضِيتَ عَنْهُمْ وَغَفَرْتَ لَهُمْ وَاسْتَجَبْتَ دُعَائِهِمْ وَكَتَبْتَ لَهُمُ الْجَنَّةَ",
    count: 3,
    audio: "https://audio.islamweb.net/audio/index.php?page=fullcontent&audioid=13766",
    category: "أدعية متنوعة",
    explanation: 'دعاء يجمع بين طلب العفو والرضا والمغفرة واستجابة الدعاء ودخول الجنة'
  },
  {
    id: '16',
    text: "اللَّهُمَّ إِنِّي أَسْأَلُكَ عِلْمًا نَافِعًا، وَرِزْقًا طَيِّبًا، وَعَمَلًا مُتَقَبَّلًا",
    count: 1,
    category: 'أدعية متنوعة',
    explanation: 'دعاء طلب العلم النافع والرزق الطيب والعمل الصالح'
  },
  {
    id: '17',
    text: "رَبَّنَا آتِنَا فِي الدُّنْيَا حَسَنَةً، وَفِي الْآخِرَةِ حَسَنَةً، وَقِنَا عَذَابَ النَّارِ",
    count: 1,
    category: 'أدعية متنوعة',
  },
  {
    id: '18',
    text: "اللَّهُمَّ أَصْلِحْ لِي دِينِيَ الَّذِي هُوَ عِصْمَةُ أَمْرِي، وَأَصْلِحْ لِي دُنْيَايَ الَّتِي فِيهَا مَعَاشِي، وَأَصْلِحْ لِي آخِرَتِي الَّتِي فِيهَا مَعَادِي، وَاجْعَلِ الْحَيَاةَ زِيَادَةً لِي فِي كُلِّ خَيْرٍ، وَاجْعَلِ الْمَوْتَ رَاحَةً لِي مِنْ كُلِّ شَرٍّ",
    count: 1,
    category: 'أدعية متنوعة',
  },
  {
    id: '19',
    text: "اللَّهُمَّ إِنِّي أَعُوذُ بِكَ مِنَ الْهَمِّ وَالْحَزَنِ، وَالْعَجْزِ وَالْكَسَلِ، وَالْبُخْلِ وَالْجُبْنِ، وَضَلَعِ الدَّيْنِ، وَغَلَبَةِ الرِّجَالِ",
    count: 1,
    category: 'أدعية متنوعة',
  },
  {
    id: '20',
    text: "اللَّهُمَّ أَنْتَ رَبِّي لا إِلَهَ إِلا أَنْتَ، خَلَقْتَنِي وَأَنَا عَبْدُكَ، وَأَنَا عَلَى عَهْدِكَ وَوَعْدِكَ مَا اسْتَطَعْتُ، أَعُوذُ بِكَ مِنْ شَرِّ مَا صَنَعْتُ، أَبُوءُ لَكَ بِنِعْمَتِكَ عَلَيَّ، وَأَبُوءُ لَكَ بِذَنْبِي فَاغْفِرْ لِي؛ فَإِنَّهُ لا يَغْفِرُ الذُّنُوبَ إِلا أَنْتَ",
    count: 1,
    category: 'أذكار الصباح والمساء',
    source: 'حصن المسلم'
  },
  {
    id: '21',
    text: "اللَّهُمَّ بِكَ أَصْبَحْنَا، وَبِكَ أَمْسَيْنَا، وَبِكَ نَحْيَا، وَبِكَ نَمُوتُ، وَإِلَيْكَ النُّشُورُ",
    count: 1,
    category: 'أذكار الصباح والمساء',
    source: 'إسلام ويب'
  },
  {
    id: '22',
    text: "رَضِيتُ بِاللَّهِ رَبًّا، وَبِالْإِسْلَامِ دِينًا، وَبِمُحَمَّدٍ ﷺ نَبِيًّا",
    count: 3,
    category: 'أذكار الصباح والمساء',
    source: 'إسلام ويب'
  },
  {
    id: '23',
    text: "اللَّهُمَّ إِنِّي أَسْأَلُكَ الْعَفْوَ وَالْعَافِيَةَ فِي الدُّنْيَا وَالآخِرَةِ، اللَّهُمَّ إِنِّي أَسْأَلُكَ الْعَفْوَ وَالْعَافِيَةَ فِي دِينِي وَدُنْيَايَ وَأَهْلِي وَمَالِي، اللَّهُمَّ اسْتُرْ عَوْرَاتِي، وَآمِنْ رَوْعَاتِي، وَاحْفَظْنِي مِنْ بَيْنِ يَدَيَّ وَمِنْ خَلْفِي، وَعَنْ يَمِينِي وَعَنْ شِمَالِي، وَمِنْ فَوْقِي، وَأَعُوذُ بِعَظَمَتِكَ أَنْ أُغْتَالَ مِنْ تَحْتِي",
    count: 1,
    category: 'أذكار الصباح والمساء',
    source: 'إسلام ويب'
  },
  {
    id: '24',
    text: "قُلْ هُوَ اللَّهُ أَحَدٌ، قُلْ أَعُوذُ بِرَبِّ الْفَلَقِ، قُلْ أَعُوذُ بِرَبِّ النَّاسِ",
    count: 3,
    category: 'أذكار الصباح والمساء',
    source: 'إسلام ويب'
  },
  {
    id: '25',
    text: "بِاسْمِكَ اللَّهُمَّ أَحْيَا وَأَمُوتُ",
    count: 1,
    category: 'أذكار النوم',
    source: 'صحيح البخاري'
  },
  {
    id: '26',
    text: "اللَّهُمَّ قِنِي عَذَابَكَ يَوْمَ تَبْعَثُ عِبَادَكَ",
    count: 1,
    category: 'أذكار النوم',
    source: 'صحيح البخاري'
  },
  {
    id: '29',
    text: "اللَّهُمَّ رَحْمَتَكَ أَرْجُو، فَلَا تَكِلْنِي إِلَى نَفْسِي طَرْفَةَ عَيْنٍ، وَأَصْلِحْ لِي شَأْنِي كُلَّهُ، لَا إِلَهَ إِلَّا أَنْتَ",
    count: 1,
    category: 'أدعية الكرب',
    source: 'إسلام ويب'
  },
  {
    id: '30',
    text: "لَا إِلَهَ إِلَّا اللَّهُ الْعَظِيمُ الْحَلِيمُ، لَا إِلَهَ إِلَّا اللَّهُ رَبُّ الْعَرْشِ الْعَظِيمِ، لَا إِلَهَ إِلَّا اللَّهُ رَبُّ السَّمَاوَاتِ وَرَبُّ الْأَرْضِ، وَرَبُّ الْعَرْشِ الْكَرِيمِ",
    count: 1,
    category: 'أدعية الكرب',
    source: 'إسلام ويب'
  },
  {
    id: '31',
    text: "لَا إِلَهَ إِلَّا أَنْتَ سُبْحَانَكَ إِنِّي كُنْتُ مِنَ الظَّالِمِينَ",
    count: 1,
    category: 'أدعية الكرب',
    source: 'إسلام ويب'
  },
  {
    id: '32',
    text: "اللَّهُمَّ إِنِّي أَسْتَخِيرُكَ بِعِلْمِكَ، وَأَسْتَقْدِرُكَ بِقُدْرَتِكَ، وَأَسْأَلُكَ مِنْ فَضْلِكَ الْعَظِيمِ، فَإِنَّكَ تَقْدِرُ وَلَا أَقْدِرُ، وَتَعْلَمُ وَلَا أَعْلَمُ، وَأَنْتَ عَلَّامُ الْغُيُوبِ. اللَّهُمَّ إِنْ كُنْتَ تَعْلَمُ أَنَّ هَذَا الْأَمْرَ (وَيُسَمِّي حَاجَتَهُ) خَيْرٌ لِي فِي دِينِي وَمَعَاشِي وَعَاقِبَةِ أَمْرِي، فَاقْدُرْهُ لِي وَيَسِّرْهُ لِي، ثُمَّ بَارِكْ لِي فِيهِ. وَإِنْ كُنْتَ تَعْلَمُ أَنَّ هَذَا الْأَمْرَ شَرٌّ لِي فِي دِينِي وَمَعَاشِي وَعَاقِبَةِ أَمْرِي، فَاصْرِفْهُ عَنِّي وَاصْرِفْنِي عَنْهُ، وَاقْدُرْ لِي الْخَيْرَ حَيْثُ كَانَ، ثُمَّ رَضِّنِي بِهِ",
    count: 1,
    category: 'أدعية الاستخارة',
    source: 'إسلام ويب',
    note: 'يقال بعد صلاة ركعتين من غير الفريضة'
  },
  {
    id: '33',
    text: "اللَّهُمَّ إِنِّي أَعُوذُ بِكَ مِنَ الْكُفْرِ وَالْفَقْرِ، وَأَعُوذُ بِكَ مِنْ عَذَابِ الْقَبْرِ، لَا إِلَهَ إِلَّا أَنْتَ",
    count: 3,
    category: 'أذكار الصباح والمساء',
    source: 'حصن المسلم'
  },
  {
    id: '34',
    text: "حَسْبُنَا اللَّهُ لَا إِلَهَ إِلَّا هُوَ، عَلَيْهِ تَوَكَّلْنَا، وَهُوَ رَبُّ الْعَرْشِ الْعَظِيمِ",
    count: 7,
    category: 'أذكار الصباح والمساء',
    source: 'حصن المسلم'
  },
  {
    id: '35',
    text: "اللَّهُمَّ إِنِّي أَعُوذُ بِكَ مِنَ الْكُفْرِ وَالْفَقْرِ، وَأَعُوذُ بِكَ مِنْ عَذَابِ الْقَبْرِ، لَا إِلَهَ إِلَّا أَنْتَ",
    count: 3,
    category: 'أذكار الصباح والمساء',
    source: 'حصن المسلم'
  },
  {
    id: '36',
    text: "حَسْبُنَا اللَّهُ لَا إِلَهَ إِلَّا هُوَ، عَلَيْهِ تَوَكَّلْنَا، وَهُوَ رَبُّ الْعَرْشِ الْعَظِيمِ",
    count: 7,
    category: 'أذكار الصباح والمساء',
    source: 'حصن المسلم'
  },
  {
    id: '39',
    text: "اللَّهُ اللَّهُ رَبِّي لَا أُشْرِكُ بِهِ شَيْئًا",
    count: 1,
    category: 'أدعية الكرب',
    source: 'إسلام ويب'
  },
  {
    id: '40',
    text: "اللَّهُمَّ إِنِّي أَعُوذُ بِكَ مِنَ الْهَمِّ وَالْحَزَنِ، وَالْعَجْزِ وَالْكَسَلِ، وَالْبُخْلِ وَالْجُبْنِ، وَضَلَعِ الدَّيْنِ، وَغَلَبَةِ الرِّجَالِ",
    count: 1,
    category: 'أدعية الهم والحزن',
    source: 'إسلام ويب'
  },

  {
    id: 'post_salat_2',
    text: ' أستغفر الله" (ثلاث مرات) ,اللَّهُمَّ أَنْتَ السَّلَامُ، وَمِنْكَ السَّلَامُ، تَبَارَكْتَ يَا ذَا الْجَلَالِ وَالْإِكْرَامِ',
    count: 1,
    category: 'أذكار ما بعد الصلاة',
    source: 'صحيح مسلم',
    explanation: 'كان النبي صلى الله عليه وسلم يقولها إذا انصرف من صلاته'
  },
  {
    id: 'post_salat_3',
    text: 'لَا إِلَهَ إِلَّا اللَّهُ وَحْدَهُ لَا شَرِيكَ لَهُ، لَهُ الْمُلْكُ وَلَهُ الْحَمْدُ وَهُوَ عَلَى كُلِّ شَيْءٍ قَدِيرٌ',
    count: 1,
    category: 'أذكار ما بعد الصلاة',
    source: 'متفق عليه'
  },
  {
    id: 'post_salat_4',
    text: 'اللَّهُمَّ لَا مَانِعَ لِمَا أَعْطَيْتَ وَلَا مُعْطِيَ لِمَا مَنَعْتَ وَلَا يَنْفَعُ ذَا الْجَدِّ مِنْكَ الْجَدُّ',
    count: 1,
    category: 'أذكار ما بعد الصلاة',
    source: 'متفق عليه'
  },
  {
    id: 'post_salat_5',
    text: 'لَا حَوْلَ وَلَا قُوَّةَ إِلَّا بِاللَّهِ، لَا إِلَهَ إِلَّا اللَّهُ، وَلَا نَعْبُدُ إِلَّا إِيَّاهُ، لَهُ النِّعْمَةُ وَلَهُ الْفَضْلُ وَلَهُ الثَّنَاءُ الْحَسَنُ، لَا إِلَهَ إِلَّا اللَّهُ مُخْلِصِينَ لَهُ الدِّينَ وَلَوْ كَرِهَ الْكَافِرُونَ',
    count: 1,
    category: 'أذكار ما بعد الصلاة',
    source: 'صحيح مسلم'
  },
  {
    id: 'post_salat_6',
    text: 'اللَّهُمَّ أَعِنِّي عَلَى ذِكْرِكَ وَشُكْرِكَ وَحُسْنِ عِبَادَتِكَ',
    count: 1,
    category: 'أذكار ما بعد الصلاة',
    source: 'سنن أبي داود والنسائي وأحمد',
    explanation: 'وصية النبي صلى الله عليه وسلم لمعاذ بن جبل'
  },
  {
    id: 'post_salat_7',
    text: 'اللَّهُمَّ إِنِّي أَعُوذُ بِكَ مِنَ الْبُخْلِ، وَأَعُوذُ بِكَ مِنَ الْجُبْنِ، وَأَعُوذُ بِكَ مِنْ أَنْ أُرَدَّ إِلَى أَرْذَلِ الْعُمُرِ، وَأَعُوذُ بِكَ مِنْ فِتْنَةِ الدُّنْيَا، وَأَعُوذُ بِكَ مِنْ عَذَابِ الْقَبْرِ',
    count: 1,
    category: 'أذكار ما بعد الصلاة',
    source: 'صحيح البخاري'
  },
  {
    id: 'post_salat_8',
    text: 'سُبْحَانَ اللَّهِ (33 مرة)، وَالْحَمْدُ لِلَّهِ (33 مرة)، وَاللَّهُ أَكْبَرُ (33 مرة)، لَا إِلَهَ إِلَّا اللَّهُ وَحْدَهُ لَا شَرِيكَ لَهُ، لَهُ الْمُلْكُ وَلَهُ الْحَمْدُ وَهُوَ عَلَى كُلِّ شَيْءٍ قَدِيرٌ (مرة واحدة لإتمام المئة)',
    count: 1,
    category: 'أذكار ما بعد الصلاة',
    source: 'صحيح مسلم',
    explanation: 'من قالها غفرت خطاياه وإن كانت مثل زبد البحر'
  },
  {
    id: 'post_salat_9',
    text: 'اللَّهُ لَا إِلَٰهَ إِلَّا هُوَ الْحَيُّ الْقَيُّومُ ۚ لَا تَأْخُذُهُ سِنَةٌ وَلَا نَوْمٌ ۚ لَهُ مَا فِي السَّمَاوَاتِ وَمَا فِي الْأَرْضِ ۗ مَنْ ذَا الَّذِي يَشْفَعُ عِنْدَهُ إِلَّا بِإِذْنِهِ ۚ يَعْلَمُ مَا بَيْنَ أَيْدِيهِمْ وَمَا خَلْفَهُمْ ۖ وَلَا يُحِيطُونَ بِشَيْءٍ مِنْ عِلْمِهِ إِلَّا بِمَا شَاءَ ۚ وَسِعَ كُرْسِيُّهُ السَّمَاوَاتِ وَالْأَرْضَ ۖ وَلَا يَئُودُهُ حِفْظُهُمَا ۚ وَهُوَ الْعَلِيُّ الْعَظِيمُ',
    count: 1,
    category: 'أذكار ما بعد الصلاة',
    source: 'آية الكرسي من سورة البقرة (255)',
    explanation: 'من قرأ آية الكرسي دبر كل صلاة لم يمنعه من دخول الجنة إلا الموت'
  },
  {
    id: 'post_salat_10',
    text: 'قُلْ هُوَ اللَّهُ أَحَدٌ ... (سورة الإخلاص)',
    count: 1,
    category: 'أذكار ما بعد الصلاة',
    source: 'المعوذات',
    explanation: 'تقرأ بعد كل صلاة، وثلاث مرات بعد صلاتي الفجر والمغرب'
  },
  {
    id: 'post_salat_11',
    text: 'قُلْ أَعُوذُ بِرَبِّ الْفَلَقِ ... (سورة الفلق)',
    count: 1,
    category: 'أذكار ما بعد الصلاة',
    source: 'المعوذات',
    explanation: 'تقرأ بعد كل صلاة، وثلاث مرات بعد صلاتي الفجر والمغرب'
  },
  {
    id: 'post_salat_12',
    text: 'قُلْ أَعُوذُ بِرَبِّ النَّاسِ ... (سورة الناس)',
    count: 1,
    category: 'أذكار ما بعد الصلاة',
    source: 'المعوذات',
    explanation: 'تقرأ بعد كل صلاة، وثلاث مرات بعد صلاتي الفجر والمغرب'
  },
  {
    id: 'post_salat_13',
    text: 'لَا إِلَهَ إِلَّا اللَّهُ وَحْدَهُ لَا شَرِيكَ لَهُ، لَهُ الْمُلْكُ وَلَهُ الْحَمْدُ يُحْيِي وَيُمِيتُ وَهُوَ عَلَى كُلِّ شَيْءٍ قَدِيرٌ',
    count: 10,
    category: 'أذكار ما بعد الصلاة (الفجر والمغرب)',
    source: 'سنن الترمذي والنسائي وأحمد',
    explanation: 'من قالها عشر مرات بعد الفجر والمغرب كانت له حرزاً من الشيطان، وكتب له بها عشر حسنات، ومحي عنه عشر سيئات، ورفع له عشر درجات، وكانت له كعتق عشر رقاب'
  },
  {
    id: 'misc_dua_1',
    text: 'اللَّهُمَّ اجْعَلْنَا مِنْ عَفَوْتَ وَرَضِيتَ عَنْهُمْ وَغَفَرْتَ لَهُمْ وَاسْتَجَبْتَ دُعَائِهِمْ وَكَتَبْتَ لَهُمُ الْجَنَّةَ',
    count: 1,
    category: 'أدعية متنوعة',
    explanation: 'هذا الدعاء يطلب من الله أن يجعلنا من المجموعة المباركة التي حصلت على العفو الإلهي، الرضا الإلهي، الغفران، استجابة الدعاء، والفوز بالجنة'
  },
  {
    id: 'misc_dua_2',
    text: 'اللَّهُمَّ اغْفِرْ لِي ذَنْبِي كُلَّهُ دَقَّهُ وَجَلَّهُ وَجَلَّهُ أَوَّلهُ وَآخِرَهُ وَعَلاَنِيَتَهُ وَمَا عَلِمْتُ مِنْهُ وَمَا لَمْ أَعْلَمْ',
    count: 1,
    category: 'أدعية متنوعة',
    explanation: 'هذا الدعاء يطلب من الله الغفران الشامل لجميع الذنوب، سواء كانت صغيرة (دقة) أو كبيرة (جلة)، مهما كان حجمها أو نوعها، سواء ظهرت للناس (علانية) أو بقيت خافية، سواء علم الإنسان بها أم لا'
  },
  {
    id: 'misc_dua_3',
    text: 'اللَّهُمَّ إِنِّي أَسْتَغْفِرُكَ مِمَّا أَعْلَمُ وَمِمَّا لَا أَعْلَمُ وَمِمَّا أَنْتَ أَعْلَمُ بِهِ إِنَّكَ أَنتَ الأَعْزَرُ الأَكْرَمُ',
    count: 1,
    category: 'أدعية متنوعة',
    explanation: 'هذا الدعاء يعبر عن التوبة الشاملة، حيث يطلب العبد الغفران لكل ما يعرفه من ذنوب، ولكل ما لا يعلمه، مع الإقرار بأن الله سبحانه وتعالى هو الأعلم بكل شيء. كما يذكر العبد رحمة الله وكرمه في قبول التوبة'
  },
  {
    id: 'misc_dua_4',
    text: 'اللَّهُمَّ أَجِرْ بِخَاطِرِي جَبْرَاءً وَلِيَّهُ فَإِنَّهُ لَا يَعْجَزُكَ شَيْئٌ فِي الأَرْضِ وَلَا فِي السَّمَاءِ يَا سَمِيعَ الدُّعَاءِ',
    count: 1,
    category: 'أدعية متنوعة',
    explanation: 'هذا الدعاء يطلب من الله الحماية والرعاية للقلب والفكر من كل شر، ويؤكد على قدرة الله العظيمة التي لا تعجز عن شيء في الأرض ولا في السماء. كما يشير إلى سماع الله للدعاء وقبوله'
  }
];